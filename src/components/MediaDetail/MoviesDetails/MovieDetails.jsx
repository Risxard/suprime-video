...
import MovieOptions from "../MovieOptions/MovieOptions.jsx";
...
var MovieDetails = (SectionData) => {
  ...
  const genres = genre_ids_to_map.map((genre) => {
    const name = genreConverter(genre.id, language, "movie");
    return { name, genreId: genre.id };
  });
    ...
    <div className="MovieContent">
      ...
      <div className="content-btns">
        <span className="IncludeBD">
          ...
          <p>{langComponents.includeWithAcaiwave.title}</p>
        </span>

        <div className="movie-details-container">
          <span className="movie-details-btns">
            <span className="play-btn" onClick={() => handleExitClick()}>
              <svg
              ...
              {buttonsLang.optionsButtons.play2}
            </span>
        ...
        {genres.length > 0 && (
          <ul className="genres">
            {genres.map((genre, index) => (
              <li key={index}>
                <Link to={`/categories/${genre.genreId}`}>
                  <p>{genre.name}</p>
                </Link>
                <span>•</span>
              </li>
