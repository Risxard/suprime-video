  ...
  return (
    <div className="detail-card-list">
      {validCards.map((card, index) => {
        ...
        const imgBase = userType === "guest" ? image_path_185 : image_path_342;

        const imgBaseLarge =
          userType === "guest" ? image_path_185 : image_path_500;

        return (
          <div key={index} className="detail-card-item">
            <Link
              to={`/detail/${mediaType}/${card.id}`}
                  ...
                  {card.still_path && (
                    <picture>
                      <source
                        ...
                        srcSet={`${imgBaseLarge}${card.still_path}`}
                      />

                      <img
                        src={`${imgBase}${card.still_path}`}
