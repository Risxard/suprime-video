...
import { bgDetect, dateConverter, filteredMediaType, runtimeConverter } from "../../../../functions/Converter";
...
export default function Backdropitem({movie, mediaType , SlideItem, language,}) {

  const newMediaType = filteredMediaType(movie, mediaType);
  const mediaTitle = newMediaType === 'movie' ? movie.title : movie.name;
  const mediaDates = newMediaType === 'movie' ? movie.release_date : movie.first_air_date;
  ...
  return (
    <li className="Slide-Item" key={movie.id} >
      <Link
      ...
      <span className="backdrop-info-card">
        ...
        <span className="backdrop-overview-card">
        {movie.overview}
        </span>
      </span>
    </li>
